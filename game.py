...
class Game:
    ...
    def get_save_resume_frame(self, coords):
        ...
        save_bt = tk.Button(mf, image=tk.save_img, width=190, height=90, borderwidth=0,
                            command=lambda: self.save_btn_handler())
        save_bt.pack(side=tk.TOP, expand=False)

        # resume btn
        img = Image.open(GameParams.RESUME_IMAGE_PATH)
        tk.resume_img = ImageTk.PhotoImage(img)
        resume_bt = tk.Button(mf, image=tk.resume_img, width=190, height=90, borderwidth=0,
                              command=lambda: self.resume_bt_handler())
    ...
    def create_widgets(self):
        ...
        # Play button
        new_bt = tk.Button(master=control_frame, text="New Game", font="Times 20 bold", borderwidth=4, relief="raised",
                           justify="center", width=10, height=1
        ...
        # Load button
        load_bt = tk.Button(master=control_frame, text="Load", font="Times 20 bold", borderwidth=4, relief="raised",
                            justify="center", width=10, height=1
        ...
        combo_box_level = ttk.Combobox(dropdown_frame, textvariable=self.saved_games_var, state='readonly',
                                       justify='center', height=100, width=50)
    ...
    def create_game_window(self):
        ...
        score_label = ttk.Label(s_label_f, font=("Helvetica", 50), justify='left', anchor='w',
                                textvariable=score_label_textvar)
